...
- **Expects**: `pipeline.config.settings` with `scan_roots`, `dp_id_exclusions`, `crawl_manifest_dir`, `crawler_version`, `registry_api_url`, `log_dir`
...
- **Guarantees**: Two-pass crawl -- manifests are written before any registry POST. Failed status derivation happens between passes (pending delivery superseded by newer version in same workplan+dp_id = failed).
...
- walk_roots stops descending once it hits msoc/msoc_new (dirnames.clear()), so nested msoc dirs would be missed (by design)
