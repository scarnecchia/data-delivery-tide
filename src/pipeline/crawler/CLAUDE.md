...
- **Guarantees**: Two-pass crawl -- manifests are written before any registry POST. Status derivation via lexicon hooks happens between passes. `walk_roots` enforces canonical 5-level traversal constrained by `target` field: only directories whose names match lexicon.dir_map keys under each dpid are descended.
